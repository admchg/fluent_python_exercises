...
def sum_n_integers(*integers):
    return sum(integers)


def sum_n_lists(*integer_lists):
    return [sum_n_integers(*integers) for integers in zip(*integer_lists)]
...
def calculate(func_name, N):
    # return eval(f"{func_name}({N})")
    return func_name(N)
...
def create_group(name, *founders, created_on=None, **tags):
    output = dict()
    output["name"] = name
...
if __name__ == "__main__":
    ...
    result = {
        ...
        "founders": ("Harry Potter", "Hermoine Granger", "Ron Weasley"),
        "created_on": None,
        "group_type": "secret",
        "purpose": "PD",
